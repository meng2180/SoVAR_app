...
class Simulator:
    ...
    def get_spawn(self):
        spawns = self.remote.command("mapper/spawn/get")
        return [Spawn.from_json(spawn) for spawn in spawns]

    @accepts((Transform, Spawn))
    def map_to_gps(self, transform):
        j = self.remote.command("mapper/to_gps", {"transform": transform.to_json()})
    ...
    def map_from_gps_batch(self, coords):
        ...
        jarr = self.remote.command("mapper/from_gps", jarr)
        transforms = []
        for j in jarr:
    ...
    def map_point_on_lane(self, point):
        j = self.remote.command("mapper/point_on_lane", {"point": point.to_json()})
